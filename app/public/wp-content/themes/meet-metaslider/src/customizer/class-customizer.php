...
use meet_metaslider_SuperbThemesCustomizer\CustomizerControls;

use meet_metaslider_SuperbThemesCustomizer\Utils\RefocusButtonControl;
use meet_metaslider_SuperbThemesCustomizer\Utils\CustomizerRefocusButton;
...
class CustomizerController
{
    ...
    private function OverwriteValues()
    {
        ...
        $wp_customize->get_section('background_image')->title = __('Site Background', 'meet-metaslider');
        $wp_customize->get_control('background_color')->section = 'background_image';

        $wp_customize->get_control('header_textcolor')->section = CustomizerPanels::COLORS.CustomizerSections::NAVIGATION;
        $wp_customize->get_control('header_textcolor')->label = __('Logo Color', 'meet-metaslider');
        $wp_customize->get_control('header_textcolor')->description =__('Sets the text colors for the logo.', 'meet-metaslider');

        $wp_customize->get_control('header_image')->section = CustomizerPanels::HEADER.CustomizerSections::HEADER_DEFAULT;
        $wp_customize->get_section(CustomizerPanels::HEADER.CustomizerSections::HEADER_DEFAULT)->title = __('Default Header', 'meet-metaslider');
    }

    public function superbthemes_customizer_preview_scripts()
    {
        wp_enqueue_script('meet-metaslider-customizer-preview', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), wp_get_theme()->Version, true);
    }

    public function superbthemes_customizer_customizer_scripts()
    {
        wp_enqueue_style('meet-metaslider-customizer-css', get_template_directory_uri() . '/css/customizer.css', array(), wp_get_theme()->Version);
    }
    ...
    */
    ...
    public function superbthemes_customizer_css_final_output()
    {
        ...
        <style type="text/css">
            ...
            <?php if (get_theme_mod(CustomizerControls::HEADER_METASLIDER_THEME_STYLE) == '1') : ?>
          ...
          <?php if (get_theme_mod(CustomizerControls::BLOGFEED_TWO_COLUMNS) == '1') : ?>
             .all-blog-articles {
                display: block;
            }
